...
export function TesterTemplate({ portfolio, getInitials }: TesterTemplateProps) {
  ...
  const softSkills = ["Attention to Detail", "Bug Analysis", "Test Planning", "Quality Assurance", "Documentation"];
          ...
          <div className="grid gap-12 lg:grid-cols-2">
            ...
            <div className="flex justify-center lg:justify-start">
              <div className="relative h-96 w-80 overflow-hidden rounded-3xl">
                {(portfolio as any).profilePicture ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-teal-600 to-cyan-600 text-6xl font-bold">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className="mb-8 leading-relaxed text-gray-400">
                {portfolio.bio || "Passionate QA engineer ensuring software quality and reliability."}
              </p>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-4xl font-bold text-teal-400">{monthsOfExperience}+</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Internship Completed</p>
                  <p className="text-4xl font-bold text-teal-400">{internshipsCompleted}+</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Projects Tested</p>
                  <p className="text-4xl font-bold text-teal-400">{projectsCompleted}+</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">No of Skills</p>
                  <p className="text-4xl font-bold text-teal-400">{skillsCount}+</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="relative">
              <div className="grid gap-6 md:grid-cols-3">
                {projects.slice(currentProjectIndex, currentProjectIndex + 3).map((project) => (
                  <div
                    key={project.id}
                    className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm transition-all hover:border-teal-500/50"
                  >
                    {/* Project Logo/Title */}
                    <div className="mb-4 flex h-32 items-center justify-center rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20">
                    ...
                    {project.technologies && project.technologies.length > 0 && (
                      <div className="mb-4 flex flex-wrap gap-2">
                        {project.technologies.slice(0, 4).map((tech, techIdx) => (
                          <Badge
                            key={techIdx}
                            className="rounded-full border-teal-500/50 bg-teal-500/10 text-xs text-teal-400"
                          >
                            {tech}
                          </Badge>
                        ))}
            ...
            <div className="relative">
              <div className="grid gap-6 md:grid-cols-3">
                {achievements.slice(currentAchievementIndex, currentAchievementIndex + 3).map((achievement) => (
                  <div
                    key={achievement.id}
                    className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm transition-all hover:border-teal-500/50"
                  >
                    {/* Achievement Image Placeholder */}
                    <div className="h-48 bg-gradient-to-br from-teal-600/20 to-cyan-600/20" />
            ...
            <div className="flex justify-center gap-6">
              {portfolio.github && (
                <a
                  ...
                  className="group flex h-16 w-16 items-center justify-center rounded-full border border-white/20 bg-white/5 backdrop-blur-sm transition-all hover:border-teal-500/50 hover:bg-teal-500/10"
                >
                  <Github className="h-8 w-8 text-white transition-colors group-hover:text-teal-400" />
                </a>
              )}
              {portfolio.linkedin && (
                <a
                  ...
                  className="group flex h-16 w-16 items-center justify-center rounded-full border border-white/20 bg-white/5 backdrop-blur-sm transition-all hover:border-teal-500/50 hover:bg-teal-500/10"
                >
                  <Linkedin className="h-8 w-8 text-white transition-colors group-hover:text-teal-400" />
                </a>
              )}
              {portfolio.twitter && (
                <a
                  ...
                  className="group flex h-16 w-16 items-center justify-center rounded-full border border-white/20 bg-white/5 backdrop-blur-sm transition-all hover:border-teal-500/50 hover:bg-teal-500/10"
                >
                  <Twitter className="h-8 w-8 text-white transition-colors group-hover:text-teal-400" />
                </a>
              )}
            </div>
          </div>
